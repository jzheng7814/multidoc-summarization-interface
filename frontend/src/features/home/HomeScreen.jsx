...
import { createRunFromCaseId, createRunFromUpload, fetchRunDefaults } from '../../services/apiClient';
...
function HomeScreen({ onStartExtraction }) {
    ...
    useEffect(() => {
        ...
        const loadDefaults = async () => {
            ...
            setDefaultsError('');
            try {
                const payload = await fetchRunDefaults();
                if (cancelled) {
                    return;
                }
                const nextExtraction = parseExtractionConfig(payload?.extractionConfig ?? payload?.extraction_config);
                const nextSummary = parseSummaryConfig(payload?.summaryConfig ?? payload?.summary_config);
                setExtractionConfig(nextExtraction);
                ...
                setDefaultsError(error?.message || 'Failed to load backend defaults.');
            } finally {
                if (!cancelled) {
    ...
    const handleLoadRun = async () => {
        ...
        try {
            const payload = intakeMode === 'caseId'
                ? await createRunFromCaseId(caseId.trim())
                : await createRunFromUpload({
            ...
            setLoadError(error.message || 'Failed to initialize run.');
        } finally {
    ...
    return (
        <div className="min-h-screen bg-[var(--color-surface-app)] p-6 text-[var(--color-text-primary)] transition-colors">
            <div className="w-full max-w-[1400px] mx-auto space-y-6">
                ...
                {defaultsError && (
                    <div className="rounded border border-[var(--color-danger-soft)] bg-[var(--color-danger-soft)] px-3 py-2 text-sm text-[var(--color-text-danger)]">
                        {defaultsError}
                    </div>
                )}
                ...
                <section className="bg-[var(--color-surface-panel)] rounded-lg shadow-md p-6 border border-[var(--color-border)] space-y-4">
                    ...
                    >
                        {isCreatingRun ? 'Initializing Run…' : 'Load Run Context'}
                    </button>
                ...
                <section className="bg-[var(--color-surface-panel)] rounded-lg shadow-md p-6 border border-[var(--color-border)] space-y-4">
                    ...
                    {!extractionConfig ? (
                        <div className="rounded border border-dashed border-[var(--color-border)] px-4 py-6 text-sm text-[var(--color-text-muted)]">
                            {isLoadingDefaults
                                ? 'Loading extraction defaults from backend...'
                                : 'Extraction defaults are unavailable. Fix the error above to continue.'}
                        </div>
                    ) : (
                        <>
                            <div className="space-y-2">
                ...
                <section className="bg-[var(--color-surface-panel)] rounded-lg shadow-md p-6 border border-[var(--color-border)] space-y-4">
                    ...
                    {!summaryConfig ? (
                        <div className="rounded border border-dashed border-[var(--color-border)] px-4 py-6 text-sm text-[var(--color-text-muted)]">
                            {isLoadingDefaults
                                ? 'Loading summary defaults from backend...'
                                : 'Summary defaults are unavailable. Fix the error above to continue.'}
                        </div>
                    ) : (
                        <>
                            <div className="space-y-2">
